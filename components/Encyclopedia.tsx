...
import { Sword, Shield, Box, Beer, Search, Hammer, Coins, ArrowRightLeft, Scale, Wallet, ArrowUpDown, ChevronUp, ChevronDown, Utensils, Home, Handshake } from 'lucide-react';
import { MenuView } from './AppSidebar';
import { WEAPONS, ARMOR_SHIELDS, GEAR_SERVICES, ANIMALS_VEHICLES, LIFESTYLE_EXPENSES } from '../data/srdData';
...
const CATEGORY_TITLES: Record<string, string> = {
    ...
    'wiki-services': 'Services & Lodging',
    'wiki-various': 'Adventuring Gear & Goods',
    'wiki-economy': 'Economy & Trade'
};

const CATEGORY_ICONS: Record<string, React.ReactNode> = {
    'wiki-weapons': <Sword className="w-8 h-8 text-red-400" />,
    'wiki-equipment': <Shield className="w-8 h-8 text-blue-400" />,
    'wiki-transports': <HorseIcon className="w-8 h-8 text-yellow-400" />,
    'wiki-services': <Beer className="w-8 h-8 text-green-400" />,
    'wiki-various': <Box className="w-8 h-8 text-purple-400" />,
...
export const Encyclopedia: React.FC<EncyclopediaProps> = ({ category }) => {
    ...
    if (category === 'wiki-economy') {
        ...
        const lodging = GEAR_SERVICES.filter(i => 
            i.subcategory === 'Lodging'
        );
                 ...
                 <div className="max-w-5xl mx-auto">
                        ...
                        <div className="bg-rpg-800 border border-rpg-700 rounded-lg p-6 shadow-xl">
                            ...
                            <div className="flex space-x-4 mb-6">
                                <input 
                                    ...
                                    className="flex-1 bg-rpg-900 border border-rpg-700 rounded p-3 text-white focus:outline-none focus:border-rpg-accent font-mono text-lg"
                                />
                                <select 
                                    value={unit}
                                    onChange={(e) => setUnit(e.target.value)}
                                    className="bg-rpg-900 border border-rpg-700 rounded p-3 text-white focus:outline-none focus:border-rpg-accent"
                                >
                            ...
                            <div className="space-y-3 bg-rpg-900/50 p-4 rounded border border-rpg-700/50">
                                <div className="flex justify-between items-center text-sm border-b border-rpg-700/50 pb-2">
                                    ...
                                    <span className="font-mono font-bold text-white">{(baseInCp / 1000).toLocaleString(undefined, { maximumFractionDigits: 3 })}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm border-b border-rpg-700/50 pb-2">
                                    <span className="text-rpg-muted">Gold (gp)</span>
                                    <span className="font-mono font-bold text-yellow-400">{(baseInCp / 100).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm border-b border-rpg-700/50 pb-2">
                                    <span className="text-rpg-muted">Silver (sp)</span>
                                    <span className="font-mono font-bold text-gray-300">{(baseInCp / 10).toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-rpg-muted">Copper (cp)</span>
                                    <span className="font-mono font-bold text-orange-400">{baseInCp.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-rpg-800 border border-rpg-700 rounded-lg p-6 shadow-xl">
                                ...
                                <div className="grid grid-cols-2 gap-4 text-center">
                                    <div className="bg-rpg-900 p-3 rounded border border-rpg-700">
                                        ...
                                        <div className="font-bold text-white">0.02 lb</div>
                                    </div>
                                    <div className="bg-rpg-900 p-3 rounded border border-rpg-700">
                                        <div className="text-xs text-rpg-muted uppercase">50 Coins</div>
                                        <div className="font-bold text-white">1.00 lb</div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="bg-rpg-800 border border-rpg-700 rounded-lg p-4 shadow-xl">
                            <h3 className="text-md font-bold text-white mb-4 flex items-center border-b border-rpg-700 pb-2">
                            ...
                            <div className="space-y-2 text-sm max-h-60 overflow-y-auto custom-scrollbar pr-2">
                                {foodDrink.map((item, i) => (
                                    <div key={i} className="flex justify-between items-center group hover:bg-rpg-900/50 p-1 rounded">
                                        ...
                                        <span className="font-mono text-yellow-500 text-xs">{formatPrice(item.price_gp)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-rpg-800 border border-rpg-700 rounded-lg p-4 shadow-xl">
                             <h3 className="text-md font-bold text-white mb-4 flex items-center border-b border-rpg-700 pb-2">
                                <Home className="w-4 h-4 mr-2 text-blue-400" /> Lodging
                            </h3>
                             <div className="space-y-2 text-sm max-h-60 overflow-y-auto custom-scrollbar pr-2">
                                {lodging.map((item, i) => (
                                    <div key={i} className="flex justify-between items-center group hover:bg-rpg-900/50 p-1 rounded">
                         ...
                         <div className="bg-rpg-800 border border-rpg-700 rounded-lg p-4 shadow-xl">
                             <h3 className="text-md font-bold text-white mb-4 flex items-center border-b border-rpg-700 pb-2">
                                <Handshake className="w-4 h-4 mr-2 text-green-400" /> Services
                            </h3>
                             <div className="space-y-2 text-sm max-h-60 overflow-y-auto custom-scrollbar pr-2">
                                {services.map((item, i) => (
                                    <div key={i} className="flex justify-between items-center group hover:bg-rpg-900/50 p-1 rounded">
                    ...
                    <div className="mt-8 bg-rpg-800 border border-rpg-700 rounded-lg overflow-hidden shadow-xl">
                        <div className="p-4 bg-rpg-900/50 border-b border-rpg-700 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-white font-serif">Lifestyle Expenses</h3>
                        ...
                        <div className="grid grid-cols-12 gap-4 p-3 bg-rpg-900/30 border-b border-rpg-700 text-xs font-bold text-rpg-muted uppercase tracking-wider">
                            <div 
                                className="col-span-3 cursor-pointer flex items-center hover:text-white transition-colors"
                        ...
                        <div className="divide-y divide-rpg-700">
                            {sortedLifestyle.map((style, idx) => (
                                <div key={idx} className="grid grid-cols-12 gap-4 p-4 hover:bg-rpg-700/30 transition-colors text-sm items-center">
                                    ...
                                    <div className="col-span-2 text-yellow-500 font-mono">
                                        {formatPrice(style.price_gp)}
                                    </div>
                                    <div className="col-span-7 text-gray-300 italic leading-relaxed">
                                        {style.description}
                                    </div>
                                </div>
                            ))}
                ...
                <div className="flex items-center justify-between mb-8 border-b border-rpg-700 pb-6">
                    ...
                    <div className="relative hidden md:block">
                         <input 
                            ...
                            className="bg-rpg-800 border border-rpg-700 rounded-full py-2 pl-4 pr-10 text-sm text-white focus:outline-none focus:border-rpg-accent w-64"
                         />
                            ...
                            <div className="divide-y divide-rpg-700">
                                {items.map((item, idx) => (
                                    <div key={idx} className="grid grid-cols-12 gap-4 p-4 hover:bg-rpg-700/30 transition-colors text-sm items-center">
                                        ...
                                        <div className="col-span-5 text-gray-300 italic">
                                            ...
                                            {item.speed_ft_per_round && (
                                                <span className="inline-flex items-center ml-2 bg-rpg-900 border border-rpg-700 px-1.5 py-0.5 rounded text-xs text-blue-300 not-italic" title={`Speed: ${item.speed_ft_per_round} ft`}>
                                                    ⚡ {item.speed_ft_per_round}
                                                </span>
                                            )}
                                            {item.carrying_capacity_lb && (
                                                <span className="inline-flex items-center ml-2 bg-rpg-900 border border-rpg-700 px-1.5 py-0.5 rounded text-xs text-green-300 not-italic" title={`Capacity: ${item.carrying_capacity_lb} lb`}>
                                                    📦 {item.carrying_capacity_lb}
                                                </span>
                                            )}
                                        </div>
